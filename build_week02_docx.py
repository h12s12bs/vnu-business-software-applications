...
def build_docx():
    ...
    # Section 3: Mission 2
    doc.add_heading("三、 實作關卡二：指令 AI 重構混亂草案為標準公文與四欄對齊決策表格 (15 分鐘)", level=1)
    p_m2 = doc.add_paragraph()
    p_m2.add_run("【任務目標】：").bold = True
    p_m2.add_run("告別手工畫表格與計算欄位冒號！以實例包中的《智慧零售企劃草案_v2_new_final_FINAL.docx》為基底，指令 AI 自主解析非結構化草案，重構為標準 Markdown 商業公文、主管核示引用框、金額強制靠右表格與待辦核選清單。\n")
    p_m2.add_run("【一步一步帶你做 SOP】：\n").bold = True
    p_m2.add_run("• 步驟 1：檢視下方【災難包原始未整理草案文本】（已內嵌於提示詞中）。\n"
                 "• 步驟 2：複製下方【任務二 Prompt】貼入 AI 對話視窗送出。\n"
                 "• 步驟 3：觀察 AI 在 3 秒內自主解析草案數字，自動計算冒號位置（|:---:| 置中、|---:| 靠右對齊）。\n"
                 "• 步驟 4：檢查產出的四欄表格（分區、門市數、報廢率、預估月效益），金額欄位是否已精準靠右對齊！")
    
    # Box for raw draft
    t_m2_raw = doc.add_table(rows=1, cols=1)
    c_m2_raw = t_m2_raw.cell(0, 0)
    c_m2_raw.width = Inches(6.5)
    set_cell_background(c_m2_raw, "FEE2E2")  # Light soft red for disaster draft
    set_cell_margins(c_m2_raw, 140, 140, 180, 180)
    p_m2_raw = c_m2_raw.paragraphs[0]
    p_m2_raw.paragraph_format.space_before = Pt(4)
    p_m2_raw.paragraph_format.space_after = Pt(4)
    p_m2_raw.paragraph_format.line_spacing = 1.15
    r_m2_raw_title = p_m2_raw.add_run("📁【災難包檔案 5 原始內容】：智慧零售企劃草案_v2_new_final_FINAL.docx\n")
    r_m2_raw_title.bold = True
    r_m2_raw_title.font.size = Pt(9.5)
    r_m2_raw_title.font.color.rgb = RGBColor(0x99, 0x1B, 0x1B)
    
    draft_m2_text = (
        "智慧零售與全台門市 POS 系統升級專案草案 (未整理版)\n"
        "發文單位：總經理室專案小組\n"
        "現況說明：目前全台共 18 家門市，北區 6 店試辦電子標籤後鮮食報廢率降至 5.1%，月省 42 萬元；"
        "中區 6 店報廢率 7.8%，預估改善可省 38 萬元；南區 6 店報廢率 8.1%，預估改善可省 40 萬元。"
        "全案硬體升級預算 18 家門市共需 153 萬元。\n"
        "預定對策：\n"
        "1. 資訊部於 9/25 前完成 POS 硬體採購合約修訂。\n"
        "2. 營運部於 10/15 前完成中南區門市教育訓練時程規劃。\n"
        "3. 全台 18 家門市預計於 11/1 正式切換上線。\n"
        "請依照標準公文格式與 Markdown 四欄表格整理，並包含核選清單。"
    )
    r_m2_raw_body = p_m2_raw.add_run(draft_m2_text)
    r_m2_raw_body.font.size = Pt(8.5)
    r_m2_raw_body.font.color.rgb = RGBColor(0x37, 0x41, 0x51)
    
    prompt_m2 = (
        "【角色設定】：你是一位擁有 10 年跨國零售與商務管理經驗的總經理特助 兼 商業營運分析師。\n"
    ...
    create_prompt_box(doc, "總經理室資深特助 兼 商業營運分析師", prompt_m2, "以真實災難草案為基底，AI 自動精確解析數據並設定 |---:| 靠右對齊。")
